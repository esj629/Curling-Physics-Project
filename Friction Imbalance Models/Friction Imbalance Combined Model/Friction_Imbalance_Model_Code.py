...
class Stone:
    ...
    def Calculate_Average_Pebble_Temperature(self, angle, n, dxs, dys, pebbles):
        width = 360/n
        temperatures = []
        average_temp = -5
        ...
        for i in range(len(pebbles)):
            ...
            # calculate angle of pebble relative to stone
            theta = np.arctan2(dx, dy)
            ...
            # determine whether pebble is in this angular section
            diff = ((theta - angle + np.pi) % (2*np.pi)) - np.pi

            if abs(diff) <= np.deg2rad(width/2):
                temperatures.append(pebbles[i].get_temp())
...
class Ice_Sheet:
    ...
    def calculate_mu(self, angle, stone, n, V, temp, t_p, t_b):
        # Part 1
        T = -5
        ...
        T_g = 5
        pi = np.pi
        kappa_g = 1.49*0.000001
        B = (k_g*T_g)/np.sqrt(pi*kappa_g*t_p)

        # Part 3
        k_i = 2.3
        T_i = 0-temp
        kappa_i = 1.23*0.000001
        C = (k_i*T_i)/np.sqrt(pi*kappa_i*t_b)
...
class Pebble:
    ...
    def Add_Temperature(self, dT):
        self.__temp = self.__temp + dT
...
class Scratch_Model(Model):
    ...
    def Calculate_Scratch_Force(self, stone_, D):
        ...
        s = np.sin(stone_.get_Velocity_Angle())
        c = np.cos(stone_.get_Velocity_Angle())
        friction = (stone_.get_coefficient2()*stone_.get_mass()*9.81)
...
class Friction_Imbalance_Model(Model):
    ...
    # methods
    def Calculate_Friction_Imbalance_Force(self, stone, icesheet, dt):
        # increase temperatures of pebbles in contact.
        pebbles = stone.get_rel_pebbles()
        pebbles_t, dxs, dys = stone.calculate_pebbles_touching_running_band(pebbles)
        N_p = len(pebbles_t)
        V = np.sqrt(stone.get_xvel()**2+stone.get_yvel()**2)
        ...
        if (N_p>0):
            ...
            for pebble in pebbles_t:
                pebble.Add_Temperature(dT)
        t_b = 0
        if(V > 0.09):
            t_p = ((pebbles[0].get_radius())*2)/V
            t_b = stone.get_width()/V
        else:
            t_b = stone.get_width()/0.01
        # pebble cooling
        # Remove pebbles that are touching the running band
        angles = stone.divide_circumference(30)
        n: int=len(angles)
        pebbles_n = [p for p in pebbles if p not in pebbles_t]
        k_i = 2.3
        kappa_i = 1.23*0.000001
        A = pebbles_n[0].get_radius()**2*np.pi
        m = 1*0.000001
        c = 2100
        delta_T_i = -5
        for pebble in pebbles_n:
            if(pebble.get_temp() == -5):
                # shouldn't go any colder than temp of ice
                pass
            else:
                # pebble cooling
                delta_T_i = 0 - pebble.get_temp()
                dT = -(2*k_i*delta_T_i)/np.sqrt(np.pi*kappa_i*t_b) *(A*dt)/(m*c)
                pebble.Add_Temperature(dT)
        ...
        if(V > 0.09):
            for i in range (x):
                # difference in coefficients of friction
                T1 = stone.Calculate_Average_Pebble_Temperature(angles[i], n, dxs, dys, pebbles_t)
                T2 = stone.Calculate_Average_Pebble_Temperature(angles[x+i], n, dxs, dys, pebbles_t)
                mu1 = icesheet.calculate_mu(angles[i], stone, n, V, T1, t_p, t_b)
                mu2 = icesheet.calculate_mu(angles[x+i], stone, n, V, T2, t_p, t_b)
                diff_mu = mu1 - mu2
                force = diff_mu * stone.get_mass() * 9.81
...
def main():
    ...
    stone.set_rel_pebbles(icesheet.get_relevant_pebbles(int(np.floor(x / icesheet.get_cellsize())), int(np.floor((y + icesheet.get_width()/2) / icesheet.get_cellsize()))))


    #answer = input("Choose A Model To Run: T, Pe, Pi, S, F (Standing for: Thermal, Pebble, Pivot, Scratch, Friction Imbalance)")
    answer = "F"
    if(answer == "T"):
        ...
        # Instantiate Friction Imbalance Model
        model = Friction_Imbalance_Model()
    ...
    def animate(frame):
        for i in range (steps):
            if(answer == "T"):
                ...
                forces = model.Calculate_Scratch_Force(stone, 0.1)
            elif(answer == "F"):
                forces = model.Calculate_Friction_Imbalance_Force(stone, icesheet, dt_s)
    ...
    #animation.save("Scratch Model Clockwise Rotation.gif", writer=PillowWriter(fps=10))
...
#run_scratch_model(0.07)

def run_friction_model():
    # the timesteps, dt_s is the one getting used here
    dt_s = 0.000167 / 2
    dt_p = 0

    # instantiate all the required variables
    icesheet = Ice_Sheet(20, 45, -5, 0, 0, 0.25)
    stone = Stone(3, 0, 1.0, 19, 0.145, 0.0635,0, 0, 0, 0.01, 1, False, 0, [-2, 0], icesheet)
    icesheet.create_pebble_field()
    icesheet.create_pebble_grid()
    stone.set_rel_pebbles(icesheet.get_relevant_pebbles(int(np.floor(0 / icesheet.get_cellsize())),int(np.floor((0 + icesheet.get_width()/2) /icesheet.get_cellsize()))))
    model = Friction_Imbalance_Model()

    # run the friction imbalance model
    # no plotting as it slows the model down
    while stone.get_angularv() > 0:
        forces = model.Calculate_Friction_Imbalance_Force(stone, icesheet, dt_s)
        stone.update_motion(dt_s, dt_p, icesheet, forces, model)
    print(stone.get_ypos())
    return stone.get_ypos()

for i in range(10):
    y = run_friction_model()