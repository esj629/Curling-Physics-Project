...
def calculate_Force_Friction_Imbalance(angles, mass, t_p, t_b, V, pebbles_temp, pebbles_t, dxs, dys, n, sectors):
    ...
    if(V > 0.09):
        ...
        #print("MAX TEMP DIFF:", np.max(T) - np.min(T))
...
def Pebble_Add_Temperature(pebble_temp, pebbles_t, V, dt, N_p, touching_stamp, heated_mask, heated_pebbles, n_heated, step):
    ...
    for i in range(N_p):
        ...
        if not heated_mask[index]:
            ...
            n_heated += 1
    if np.any(pebble_temp >= 0):
        print("ZERO/POSTIVE PEBBLE TEMP - not good at all...")
    return n_heated
@njit
...
def main(D, mass, Vel, w, radiusb, density, prad, ice_temp, angularv):
    ...
    return xpos, ypos
for i in range(10):
    xpos, ypos = main(D=0.21, mass = 19.24, Vel=3, w=0.005, radiusb=0.0635, density=1, prad=0.00065, ice_temp=-5, angularv=1)
    print(ypos)