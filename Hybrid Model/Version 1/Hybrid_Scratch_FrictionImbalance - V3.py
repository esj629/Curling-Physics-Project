...
def main(D, mass, Vel, w, radiusb, density, prad, ice_temp, angularv):
    ...
    pebbles_temp = np.full(n_pebbles, ice_temp) # array of pebble temperatures - initially -5 degrees C and heated through contact with stone
    ...
    dxs = np.empty(n_pebbles) # distance from stone centre to pebble in x direction
    dys = np.empty(n_pebbles) # distance from stone centre to pebble in y direction
...
times = []
factor_values = np.arange(1, 8.1, 0.5)

# CSV file
with open("pebble_density_parameter_study_hybrid_antic_V2.csv", "w", newline="") as file:
